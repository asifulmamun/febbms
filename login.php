...
<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
        // User Info Matching
        $stmt = $conn->prepare("SELECT * FROM becomedonor WHERE mobile=? OR email=?");
        $stmt->bind_param('ss', $ume, $ume);
        ...
        $mobile = $rows['mobile']; // Store Username
        $stmt->close(); // user

        // pass & username Matching
        $stmts = $conn->prepare("SELECT * FROM `becomedonor` WHERE `mobile`=? AND `password`=?");
        $stmts->bind_param('ss', $mobile, $pass);
        $pass = md5($_POST['pass']); // get pass and encrypted
        $stmts->execute();
